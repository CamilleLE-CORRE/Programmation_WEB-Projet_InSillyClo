...
def build_excel_response(template, filename):
    ...
    # Build rows depending on template type
    rows = []
    ...
    # Headers and typed-specific content
    ...
    # Vertical border between columns B(2) and C(3)
    ...
    # Draw vertical borders for rows 15..50.
...
def create_template(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Create and save the template, then redirect to the list
...
def replace_template_xlsx(request, template_id):
    ...
    if request.method == 'POST' and request.FILES.get('xlsx'):
        ...
        data = parse_template_xlsx(xlsx)

        template.source_file = xlsx
        template.name = data["name"]
        template.separator = data["separator"]
        template.restriction_enzyme = data["restriction_enzyme"]
...
def parse_template_xlsx(xlsx_file):
    wb = load_workbook(xlsx_file, data_only=True)
    ws = wb.active

    # Métadonnées
    restriction_enzyme = ws["B2"].value
    name = ws["B3"].value
    separator = ws["B4"].value

    # Type
    header_row = [cell.value for cell in ws[9]]

    if any(v and "input Plasmid" in str(v) for v in header_row):
        template_type = "simple"
    elif any(v in ("ConL", "Promoter", "Gene") for v in header_row):
        template_type = "typed"
    
    return {
        "name": str(name).strip(),
        "separator": str(separator).strip() if separator else "",
